      ...
      >
        ...
        <div class="comment-meta">
          ...
          <div class="comment-info">
            ...
            <!-- 点赞 -->
            <!-- 点赞内容设计：commentLikeSet是与当前登录用户关联的评论点赞集合，isLike判断当前评论是否在这个用户点赞的评论ID集合中，
            如果在样式就表示为like-active，否则样式为like -->
            <!-- 点赞按钮点击的逻辑是这样的，如果用户没有登陆，就提示当前用户登录；如果用户已经登录了，前端逻辑是这样的，
            如果这条评论用户已经点过赞了，就将评论的点赞总数-1，如果没有点过赞，就将评论的点赞总数+1。最后修改commentLike集合。 -->
            <!-- api接口发送用户id和评论id，查询当前用户的点赞集合，如果包含该条评论，就remove, 不包含该条评论就insert,同时更新评论表中
            该条评论的likeCount字段（更新用户表和评论表） -->
            <span
...
export default {
  ...
  methods: {
    ...
    insertComment() {
      ...
      this.commentContent = "";
      this.axios
        .post("addcomment/" + this.$store.state.userId, comment)
        .then(({ data }) => {
          if (data.status === 200) {
            //发送重新加载评论的信号，文章详情页面重新获取评论
    ...
    like(comment) {
      ...
      this.axios
        .post("comments/like/" + comment.id, postdata)
        .then(({ data }) => {
          if (data.status === 200) {
            //判断是否点赞
    ...
    reloadReply(index) {
      this.axios
        .get("comments/replies/" + this.commentList[index].id, {
          params: {
