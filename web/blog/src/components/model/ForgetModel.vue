      ...
      <div class="login-wrapper">
        ...
        <div class="mt-7 send-wrapper">
          <v-text-field
            ...
            @keyup.enter="forget"
          />
          <v-btn :disabled="flag" te  xt small @click="sendCode">
            {{ codeMsg }}
          </v-btn>
        </div>
        <!-- 密码 -->
...
export default {
  ...
  methods: {
      ...
      var captcha = new TencentCaptcha(this.config.TENCENT_CAPTCHA, function (
        ...
        if (res.ret === 0) {
          ...
          that.axios
            ...
            .then(({ data }) => {
              if (data.status === 200) {
                that.$toast({ type: "success", message: "发送邮件成功!" });
              } else {
                that.$toast({ type: "error", message: "发送邮件失败!" });
              }
            });
        }
      });
    ...
    forget() {
      ...
      this.axios.put("users/password", user).then(({ data }) => {
        if (data.status === 200) {
  ...
  watch: {
    username(value) {
      var reg = /^[A-Za-z0-9\u4e00-\u9fa5]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/;
      if (reg.test(value)) {
        // 如果已经开始计时了，说明发送验证码时间不足60s
        if (this.time === 60) {
          clearInterval(this.timer);
          this.codeMsg = "发送";
          this.time = 60;
          this.flag = false; // 有点小bug
        }
      } else {
        this.flag = true;
      }
