...
export default {
  created() {
    ...
    this.id = parseInt(path.substr(pos + 1));
  },
  data: function () {
    return {
      current: 1,
  ...
  methods: {
    infiniteHandler($state) {
      ...
      if (this.isCategory) {
        // 根据id获取分类名
        this.axios.get(`category/${this.id}`).then(({ data }) => {
          // console.log(data);
          this.name = data.data.name;
        });
        // 处理分类文章的加载
        this.axios
          .get(`article/list/${this.id}`, {
            params: {
          ...
          .then(({ data }) => {
            ...
            if (data.data.length) {
              ...
              this.name = data.data[0].Category.name;
        ...
        // 根据标签id获取标签名
        this.axios.get(`tag/${this.id}`).then(({ data }) => {
          // console.log(data);
          this.name = data.data.name;
        });
        // 处理标签下对应文章的加载
        this.axios
          .get(`article/listByTag/${this.id}`, {
            params: {
              pagesize: this.queryParams.pageSize,
              pagenum: this.queryParams.pageNum,
            },
          ...
          .then(({ data }) => {
            ...
            if (data.data.length) {
              this.queryParams.pageNum++;
              // this.name = data.data[0].tags[0].name;
              document.title = this.title + " - " + this.name;
              this.articleList.push(...data.data);
              $state.loaded();
            } else {
