...
export default new Vuex.Store({
  ...
  mutations: {
    login(state, user) {
      ...
      state.articleLikeSet = user.articleLikeSet ? user.articleLikeSet : []; // 尚未处理
      state.commentLikeSet = user.commentLikeSet ? user.commentLikeSet : []; // 尚未处理
      // state.email = user.username;
      // state.loginType = user.loginType; // 尚未处理
    },
    logout(state) {
      state.userId = null;
      state.avatar = null;
    ...
    storeAllArts(state, allArts) {
      state.allArts = allArts;
    },
    closeModel(state) {
