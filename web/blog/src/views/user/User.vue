      ...
      <v-row class="info-wrapper">
        <v-col md="3" cols="12">
          ...
          <avatar-cropper
            ...
            trigger="#pick-avatar"
            upload-url="https://rayoluo.top/api/v1/users/avatar/upload"
          />
        </v-col>
        <v-col md="7" cols="12">
          <v-text-field
...
export default {
  ...
  // created() {
  //   this.uploadUrl = this.config.UPLOAD_URL + this.userInfo.userId;
  // },
  methods: {
    updataUserInfo() {
      this.axios
        .put("users/update/info/" + this.userInfo.userId, {
    ...
    uploadAvatar(data) {
      if (data.status === 200) {
        this.$store.commit("updateAvatar", data.url);
        this.$toast({ type: "success", message: data.message });
      } else {
        this.$toast({ type: "error", message: data.message });
      }
