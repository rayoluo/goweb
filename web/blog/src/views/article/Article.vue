      ...
      <div class="article-info-container">
        ...
        <div class="article-info">
          ...
          <div class="third-line">
            ...
            <!-- 阅读量 -->
            <span>
              <i class="iconfont iconliulan" /> 阅读量: {{ article.read_count }}
            </span>
            <span class="separator">|</span>
            <!-- 评论量 -->
            <span>
      ...
      <v-col md="9" cols="12">
        <v-card class="article-wrapper">
          ...
          <div class="aritcle-copyright">
            <div>
              <span>文章作者：</span>
              <a href="https://blog.csdn.net/hbhgyu" target="_blank">Rayoluo</a>
            </div>
            <div>
              <span>文章链接：</span>
          ...
          <div class="article-reward">
            ...
            <a :class="isLike" @click="like">
              ...
              <span v-show="articleLikeCount > 0">{{ articleLikeCount }}</span>
            </a>
            <a class="reward-btn">
              <!-- 打赏按钮 -->
          ...
          <div class="pagination-post">
            <!-- 上一篇 -->
            <div
              :class="isFull(lastArticle.id + 1)"
              v-if="lastArticle.id != -1"
            >
              <router-link :to="'/articles/' + lastArticle.ID">
                <img class="post-cover" :src="lastArticle.img" />
                <div class="post-info">
            ...
            <div
              :class="isFull(nextArticle.id - 1)"
              v-if="nextArticle.id != -1"
            >
              <router-link :to="'/articles/' + nextArticle.ID">
                <img class="post-cover" :src="nextArticle.img" />
          ...
          <!-- 推荐文章 -->
          <div class="recommend-container" v-if="articleRecommendList.length">
            ...
            <div class="recommend-list">
              ...
              >
                <router-link :to="'/articles/' + item.ID">
                  <img class="recommend-cover" :src="item.img" />
...
export default {
  ...
  methods: {
    getArticle() {
      ...
      this.id = fullpath.substr(pos + 1);
      // console.log(this.id);
      const that = this;
      //查询文章
      this.axios.get(`article/info/${this.id}`).then(({ data }) => {
    ...
    async getArticleLikeCount() {
      const { data: res } = await this.axios.get(
        `articles/likeCount/${this.id}`
      );
      this.articleLikeCount = res.likeCount;
    },
    // 加载当前文章的上一篇文章和下一篇文章信息+推荐列表
    async getRelateInfo() {
      console.log(this.$store.state.allArts)
      var len = this.$store.state.allArts.length;
      var i = 0;
      for (i = 0; i < len; i++) {
        // console.log(this.$store.state.allArts[i].ID)
        if (this.$store.state.allArts[i].ID === parseInt(this.id)) {
          break;
        }
      }
      if (i > 0) {
        this.lastArticle.id = i - 1;
        this.lastArticle.img = this.$store.state.allArts[i - 1].img;
        this.lastArticle.title = this.$store.state.allArts[i - 1].title;
        this.lastArticle.ID = this.$store.state.allArts[i - 1].ID;
      } else {
        this.lastArticle.id = -1;
      }
      if (i <= len - 2) {
        this.nextArticle.id = i + 1;
        this.nextArticle.img = this.$store.state.allArts[i + 1].img;
        this.nextArticle.title = this.$store.state.allArts[i + 1].title;
        this.nextArticle.ID = this.$store.state.allArts[i + 1].ID;
      } else {
        this.nextArticle.id = -1;
      }
      // 获取最新文章
      for (i = 0; i < 5 && i < len; i++) {
        this.articleLatestList.push(this.$store.state.allArts[i]);
      }
      // 获取推荐文章
      const { data: res } = await this.axios.get(
        `article/recommend/${this.id}`
      );
      // this.articleRecommendList = res.data;
      var count = 0;
      for (i = 0; i < res.data.length && count < 3; i++) {
        if (res.data[i].ID == parseInt(this.id)) continue;
        this.articleRecommendList.push(res.data[i]);
        count++;
      }
      if (this.articleRecommendList.length < 3) {
        this.articleRecommendList.splice(0, this.articleRecommendList.length);
      }
    },
    async getArticleTag() {
      const { data: res } = await this.axios.get(`tags/${this.id}`);
      this.tags = res.data;
    },
    // todo 修改这个方法使得能够加载所有的评论
    listComment() {
    ...
    like() {
      ...
      this.axios
        .post("articles/like/" + this.article.ID, postdata)
        .then(({ data }) => {
          if (data.status === 200) {
            //判断是否点赞
            if (
  ...
  computed: {
    ...
    isFull() {
      return function (id) {
        return id == 0 || id == this.$store.state.allArts.length - 1
          ? "post full"
          : "post";
      };
    },
