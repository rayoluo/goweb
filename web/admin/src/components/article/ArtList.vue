  ...
  <div>
    <a-card>
      <a-row :gutter="20">
        ...
        <a-col :span="4">
          <a-select placeholder="请选择分类" style="width: 160px" @change="CateChange">
      ...
      >
        ...
        <template slot="taglist" slot-scope="taglist">
          <a-tag color="#2db7f5" v-for="tag in taglist" :key="tag.id">{{ tag.name }}</a-tag>
        </template>
...
export default {
  ...
  methods: {
    ...
    handleTableChange(pagination, filters, sorter) {
      var pager = { ...this.pagination } // 解构赋值
      pager.current = pagination.current
      pager.pageSize = pagination.pageSize
      this.queryParam.pagesize = pagination.pageSize
      this.queryParam.pagenum = pagination.current
      // 如果页面大小发生改变了就将当前页数设置为1
    ...
    deleteArt(id) {
      this.$confirm({
        ...
        onOk: async () => {
          ...
          if (res.status != 200) return this.$message.error(res.message)
          // todo 删除当前文章的所有标签对应关系
          const { data: res2 } = await this.$http.delete(`articleTag/art/${id}`)
          if (res2.status !== 200) return this.$message.error('删除当前文章的标签对应关系失败!')
